...
const TABS = ['Featured', 'Homes', 'Townhomes', 'Condos', 'Mortgages', 'Statistics', 'Trends', 'New Projects', 'Commercial', 'Favourites'];
    ...
    >
      {!videoError ? (
        ...
        <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50 p-4 text-center">
          ...
          <p className="text-gray-400 text-xs font-semibold px-2">Video Unavailable</p>
        </div>
      )}
      ...
      <div className="flex flex-col gap-6 mb-8 border-b border-gray-100 pb-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            {TABS.map((tab) => (
             <Button
                ...
                className={cn(
                  ...
                  activeTab === tab ? "bg-[#06422d] hover:bg-[#06422d]/90" : "text-black"
                )}
              >
                {tab}
              </Button>
          ...
          <div className="hidden sm:flex items-center gap-2">
            <button onClick={() => scroll('left')} className="p-2 hover:bg-gray-50 rounded-full border border-gray-100">
              <ChevronLeft size={18} className="text-gray-600" />
            </button>
            <button onClick={() => scroll('right')} className="p-2 hover:bg-gray-50 rounded-full border border-gray-100">
        ...
        {filteredReels.length > 0 ? (
          filteredReels.map((reel) => (
            ...
            <p className="text-gray-500 text-sm max-w-xs text-center mt-1">
              There are currently no videos available for <span className="text-[#06422d] font-semibold">{activeTab}</span>.
            </p>
          </div>
        )}
      </div>
