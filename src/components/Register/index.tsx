...
import { Eye, EyeOff, ArrowRight, CheckCircle2, Check, X, Loader2 } from 'lucide-react';
...
export default function Register() {
    ...
    const isFormValid = validations.firstName && validations.lastName && validations.email && allPasswordMet && validations.agreeToTerms;
    ...
    return (
        ...
        <div className="hidden lg:flex lg:w-1/2 relative bg-slate-900 items-center justify-center overflow-hidden">
            ...
            <div className="relative z-10 p-12 max-w-xl text-white">
                ...
                <h1 className="text-5xl font-extrabold mb-6 leading-tight">Elevate your living <br /> <span className="text-accent font-accent italic font-medium text-4xl leading-tight">in the Sea to Sky.</span></h1>
        ...
        <div className="flex-1 flex flex-col justify-center px-6 py-12 lg:px-20 xl:px-32">
            ...
            <Link href="/login" className="ml-1 font-semibold text-primary hover:text-accent underline-offset-4 hover:underline">
                Sign in
            </Link>
            </p>
            <form onSubmit={handleSubmit} className="mt-10 space-y-5">
                ...
                <div className="space-y-1.5">
                    ...
                    <input name="firstName" className={getFieldStatus(validations.firstName, formData.firstName).className} placeholder="Alex" onChange={handleChange} />
                    {(formData.firstName.length > 0 || triedToSubmit) && !validations.firstName && (
                ...
                <div className="space-y-1.5">
                    ...
                    <input name="lastName" className={getFieldStatus(validations.lastName, formData.lastName).className} placeholder="Rivera" onChange={handleChange} />
                    {(formData.lastName.length > 0 || triedToSubmit) && !validations.lastName && (
                    <div className="flex items-center gap-1.5 text-[11px] text-destructive px-1">
                        <X size={12} /> <span>At least 2 characters</span>
                    </div>
                    )}
                ...
                <input name="email" className={getFieldStatus(validations.email, formData.email).className} placeholder="alex@example.com" onChange={handleChange} />
                {(formData.email.length > 0 || triedToSubmit) && (
                    <div className="flex items-center gap-1.5 text-[11px] px-1">
                    {validations.email ? (
                        <><Check size={12} className="text-emerald-600" /> <span className="text-emerald-700">Valid email format</span></>
                    ) : (
                ...
                <div className="relative">
                    ...
                    name="password" 
                    className={getFieldStatus(allPasswordMet, formData.password).className} 
                ...
                {(formData.password.length > 0 || triedToSubmit) && (
                    <div className={`mt-3 grid grid-cols-1 gap-1.5 p-3 rounded-xl border ${triedToSubmit && !allPasswordMet ? 'bg-destructive/5 border-destructive/20' : 'bg-slate-50/50 border-border'}`}>
                    {passwordRequirements.map((req, i) => (
                        <div key={i} className="flex items-center gap-2 text-[11px]">
                        {req.met ? <Check size={12} className="text-emerald-600" /> : <X size={12} className={triedToSubmit ? "text-destructive" : "text-slate-400"} />}
                        <span className={req.met ? "text-emerald-700 font-medium" : triedToSubmit ? "text-destructive" : "text-slate-500"}>{req.label}</span>
                        </div>
                    ))}
                    </div>
                )}
                ...
                <div className="space-y-4 pt-2">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <input 
                            ...
                            triedToSubmit && !validations.agreeToTerms ? 'ring-2 ring-destructive animate-shake' : ''
                            }`} 
                            onChange={handleChange} 
                        />
                        <label htmlFor="terms" className={`ml-3 text-xs leading-normal ${
                            triedToSubmit && !validations.agreeToTerms ? 'text-destructive font-medium' : 'text-muted-foreground'
                        }`}>
                            I have read 
                ...
                <Button type="submit" size="lg" disabled={loading} className="w-full h-14 px-8 text-lg">
